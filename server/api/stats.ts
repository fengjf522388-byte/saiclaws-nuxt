...
// Nuxt 自动将 server/api/ 下的文件映射为 API 路由
// GET /api/stats
export default defineEventHandler(async () => {
  ...
  let srsDue = 0
  cards?.forEach((c: any) => {
    const next = reviewMap[c.id]
    if (!next || new Date(next) <= new Date()) srsDue++
  })
  ...
  return {
    ...
    todayMemos: memos || 0,
    srsDue,
    pomoToday: sessions?.reduce((s: number, r: any) => s + (r.pomodoro_count || 0), 0) || 0,
    pomoMinutes: sessions?.reduce((s: number, r: any) => s + (r.duration_minutes || 0), 0) || 0
  }
})
